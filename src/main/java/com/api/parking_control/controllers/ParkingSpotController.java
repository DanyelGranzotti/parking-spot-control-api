...
public class ParkingSpotController {
    ...
    public ResponseEntity<Object> saveParkingSpot(@RequestBody @Valid ParkingSpotDto parkingSpotDto) {
        if (parkingSpotService.existsByLicensePlateCar(parkingSpotDto.getLicensePlateCar())) {
            return new ResponseEntity<>("License plate already exists", HttpStatus.BAD_REQUEST);
        }
        if (parkingSpotService.existsByParkingSpotNumber(parkingSpotDto.getParkingSpotNumber())) {
            return new ResponseEntity<>("Parking spot number already exists", HttpStatus.BAD_REQUEST);
        }
        if (parkingSpotService.existsByApartmentAndBlock(parkingSpotDto.getApartment(), parkingSpotDto.getBlock())) {
            return new ResponseEntity<>("Parking spot already registered for this apartment and block", HttpStatus.BAD_REQUEST);
        }
